...
- **Blue Screen of Death (BSOD)**: Вывод детального дампа регистров процессора (`EIP`, `EAX`, `EBX`, `ECX`, `EDX`, `ESP`, `EBP`, `CS`, `CR2` для Page Fault) и `EBP`-цепочки stack trace при возникновении критических исключений (Divide by Zero, Page Fault, GPF). Дамп автоматически дублируется на COM1 для headless-диагностики.
- **Ring-3 пользовательские процессы**: ELF-программы выполняются в нерпивилегированном кольце защиты 3 с отдельными пользовательским и ядерным стеками.
- **Вытесняющий round-robin планировщик**: Переключение задач по таймеру (time slicing), очередь готовых процессов и фоновый запуск через `spawn`.
- **Интерфейс системных вызовов**: Поддержка системных вызовов (`sys_exit`, `sys_putchar`, `sys_print`, `sys_clear`, `sys_setcursor`, `sys_getkey`, `sys_yield`, `sys_getpid`, `sys_open`, `sys_read`, `sys_write`, `sys_close`) для взаимодействия пользовательских программ с ОС.
...
- **Пользовательские ELF-программы** (в папке `src/elf`):
  - `fileview.elf` - просмотр файлов через системные вызовы.
  - `keytest.elf` - интерактивный тест кодов клавиш.
  - `gpf_test.elf` - тест обработки General Protection Fault из ring 3.
  - `spin.elf` - фоновая задача для проверки планировщика.

### Управление процессами
- **`run <file.elf>`**: Загружает и запускает ELF-программу в ring 3, ожидая её завершения.
- **`spawn <file.elf>`**: Запускает ELF-программу в фоновом режиме; shell остаётся доступен.
- **`ps`**: Выводит список активных процессов (PID, состояние, имя).
- **`kill <pid>`**: Принудительно завершает фоновый процесс.
...
Перед общей сборкой образа рекомендуется скомпилировать пользовательские ELF-приложения. Все batch-скрипты (`compile_programs.bat`, `build.bat`, `start.bat`) запускаются из корня проекта:
```bash
compile_programs.bat
```

### Инструкция по сборке и запуску ядра
Для сборки ядра, создания CawFS-структуры диска и запуска выполните из корня проекта:
```bash
